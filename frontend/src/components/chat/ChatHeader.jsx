...
const ChatHeader = ({ friend, onBack, isOnline, messages, onSelectMessage }) => {
  ...
  const handleSelectMessage = (messageId) => {
    console.log('📨 ChatHeader received select message request:', messageId);
    if (onSelectMessage) {
      onSelectMessage(messageId);
    } else {
      console.warn('⚠️ onSelectMessage prop is not defined in ChatHeader');
    }
  };

  
  return (
    <div className="flex items-center justify-between gap-3 p-4 bg-white border-b dark:bg-gray-900 dark:border-gray-800">
