...
exports.rejectFriendRequest = async (req, res) => {
  ...
  try {
    ...
    // 1. Remove the request from friendRequests array
    user.friendRequests = user.friendRequests.filter(
      (req) => req.userId.toString() !== requestId && req._id?.toString() !== requestId
    );

    // 2. Add notification with proper notificationType property
    user.notifications.push({
      notificationType: "system", // MUST match one of your enum values
...
exports.getFriendRequests = async (req, res) => {
  try {
    // Extract userId from the decoded JWT token set by your authenticate middleware
...
// Suggestions: users who are not the current user and are not already friends
exports.getSuggestions = async (req, res) => {
  try {
    const currentUserId = req.user.id || req.user._id;
    const limit = parseInt(req.query.limit, 10) || 10;

    // Fetch potential candidates (excluding self and existing friends)
    const suggestions = await User.find({
      _id: { $ne: currentUserId },
      friends: { $ne: currentUserId },
    })
      .select("firstname lastname username profilePicture friendRequests")
      .limit(limit)
      .lean();

    // Map candidates to attach the isPending status
    const formattedSuggestions = suggestions.map((candidate) => {
      const isPending = (candidate.friendRequests || []).some(
        (request) => request.userId?.toString() === currentUserId.toString()
      );
